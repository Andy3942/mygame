...
int lua_my_manager_DataManager_getShareData(lua_State* tolua_S)
{
    int argc = 0;
    ...
    if (!cobj) 
    {
        tolua_error(tolua_S,"invalid 'cobj' in function 'lua_my_manager_DataManager_getShareData'", nullptr);
        return 0;
    }
#endif
    ...
    if (argc == 0) 
    {
        if(!ok)
        {
            tolua_error(tolua_S,"invalid arguments in function 'lua_my_manager_DataManager_getShareData'", nullptr);
            return 0;
        }
        my::Map* ret = cobj->getShareData();
        object_to_luaval<my::Map>(tolua_S, "my.Map",(my::Map*)ret);
        return 1;
    }
    luaL_error(tolua_S, "%s has wrong number of arguments: %d, was expecting %d \n", "my.DataManager:getShareData",argc, 0);
    return 0;

#if COCOS2D_DEBUG >= 1
    tolua_lerror:
    tolua_error(tolua_S,"#ferror in function 'lua_my_manager_DataManager_getShareData'.",&tolua_err);
#endif
...
int lua_register_my_manager_DataManager(lua_State* tolua_S)
{
    ...
    tolua_beginmodule(tolua_S,"DataManager");
        tolua_function(tolua_S,"getShareData",lua_my_manager_DataManager_getShareData);
